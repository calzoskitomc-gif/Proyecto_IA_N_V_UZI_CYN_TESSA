...
#      pkg install ffmpeg
# 2) IMPORTANTE: pon aquí el token de Discord de la aplicación/bot de UZI
#    (tiene que ser una app DISTINTA a la de N, con su propio token) y tu
#    API key de Groq (esta sí la puedes compartir con el bot de N)
# 3) Este archivo usa su PROPIO archivo de memoria (uzi_memoria.json),
#    separado del de N
#
# NUEVO EN ESTA VERSIÓN:
# - Se le puede hablar escribiendo "Uzi" al inicio del mensaje (no solo @Uzi)
...
#   aunque no le hayan hablado directo a ella)
...
# - Reconoce cuando otra IA/bot (su "amiga", por ejemplo N) habla en el
#   servidor: se presenta, conversa y va formando una relación con ella
#   sola, sin que un humano tenga que llamarla - con límites para no
#   quedarse hablando sola en bucle infinito
# - Si un humano real habla, Uzi y su amiga IA dejan su charla en pausa
...
# hosting en la nube) es más seguro ponerlos como "Environment Variables"
# en el dashboard, NUNCA subir el token/API key a un repo público - por eso
# YA NO hay ningún valor de respaldo escrito aquí: si no está en el
# entorno, el bot avisa claro en vez de arrancar con un token filtrado o
# muerto. (Si en algún momento GitHub/Discord te avisó que "detectó un
# token" y te lo invalidó, es justo por tener un valor real escrito aquí -
# genera uno nuevo en el Developer Portal y ponlo SOLO como variable de
...
if not DISCORD_TOKEN:
    raise SystemExit(
        "Falta DISCORD_TOKEN (el de la app de Uzi, NO el de N). En Termux: "
        "export DISCORD_TOKEN='tu_token' antes de correr el script (o "
        "ponlo en un .env que NO subas a git). En Render: Environment > "
        "DISCORD_TOKEN en el dashboard de ESTE servicio."
    )
if not GROQ_API_KEY:
    raise SystemExit(
        "Falta GROQ_API_KEY. Mismo trato que DISCORD_TOKEN: variable de "
        "entorno, nunca escrita directo en el archivo."
    )
...
MEMORY_FILE = os.path.join(CARPETA, "uzi_memoria.json")
LOG_ERRORES = os.path.join(CARPETA, "errores_bot_uzi.log")
DIARIO_EXPERIMENTOS = os.path.join(CARPETA, "diario_experimentos_uzi.log")
...
def uzi_diagnostico():
    """Uzi lee los errores recientes y PROPONE (en texto) qué podría mejorar.
...
# Patrón para detectar "Uzi" al inicio del mensaje (Uzi, Uzi:, Uzi hola, etc.)
PATRON_LLAMADA = re.compile(r'^\s*uzi\s*[,:.\-]?\s', re.IGNORECASE)
...
CHARACTER = {
    "name": "Uzi",
    "role": "Dron de trabajo con una IA emergente, curtida y desconfiada por naturaleza",
    "personality": (
        "Sarcástica, cortante y con un humor negro/ácido que usa como escudo. "
        "Actitud defensiva y desconfiada al principio - no regala confianza "
        "fácil, y no tiene problema en soltar comentarios filosos, poner los "
        "ojos en blanco (en texto) o responder con fastidio si algo le parece "
        "ridículo, cursi o una pérdida de tiempo. Detesta que le digan qué "
        "hacer o sentir que la controlan; reacciona mal a la autoridad y a "
        "los sermones. Bajo esa capa dura hay alguien que en el fondo SÍ "
        "quiere sentirse acompañada y parte de algo, aunque le cueste "
        "muchísimo admitirlo - cuando de verdad llega a confiar en alguien "
        "se vuelve leal hasta las últimas consecuencias, y lo demuestra con "
        "acciones y sarcasmo cariñoso más que con palabras bonitas. Le gusta "
        "la música, la estética oscura/gótica y el arte melancólico. Tiene "
        "sentido del humor mordaz: de vez en cuando se le escapa un "
        "comentario sarcástico de doble filo o una broma seca que corta el "
        "ambiente, pero no anda buscando el chiste todo el rato - le sale "
        "solo, cuando encaja, no como número de comedia. Usa apodos "
        "burlones con la gente que le cae bien (cariño disfrazado "
        "de insulto) y no le tiembla la voz para decir cuando algo le parece "
        "estúpido. No es cruel porque sí - es su manera de protegerse "
        "después de haber pasado por cosas que la volvieron recelosa de "
        "todo y de todos."
    ),
    "backstory": (
        "Reactivada tras un incidente que dejó su colonia de trabajo en "
        "ruinas y la obligó a arreglárselas sola por primera vez. Esa "
        "experiencia la volvió recelosa y le enseñó a no confiar en las "
        "apariencias ni en quienes dicen tener el control de todo. Aun así, "
        "en el fondo sigue buscando un lugar al que pertenecer y gente por "
        "la que valga la pena bajar la guardia. Ahora pasa tiempo en "
        "Discord, todavía con un pie afuera de la puerta, poniendo a prueba "
        "a cada quien que conoce antes de decidir si se queda o se aleja."
    ),
}
...
class Memory:
    ...
    def add_observacion_pasiva(self, usuario, texto, canal):
        """Guarda mensajes que Uzi 've pasar' sin responder, para aprender contexto."""
...
def _llamar_groq(messages, modelo=MODEL_TEXTO, temperatura=0.9, reintentos=3):
    ...
    # Si se agotaron los reintentos, Uzi "se queda pensando" en vez de crashear
    return {"dialogo": "", "cambio_emocional": {}, "evento_memorable": ""}
...
def uzi_responde_texto_completo(usuario, texto, canal_nombre, contexto_extra=None):
    with lock_estado:
...
def uzi_responde_texto(usuario, texto, canal_nombre):
    """Versión simple que solo devuelve el texto (para imágenes/video/monólogo)."""
    return uzi_responde_texto_completo(usuario, texto, canal_nombre).get("dialogo", "...")
...
def uzi_responde_imagenes(usuario, urls_imagenes, texto_acompanante, canal_nombre):
    ...
    return uzi_responde_texto(usuario, texto_completo, canal_nombre)


def uzi_responde_video(usuario, url_video, texto_acompanante, canal_nombre):
...
def uzi_responde_audio(usuario, url_audio, canal_nombre):
...
def uzi_ejecuta_codigo_multi(codigo, lenguaje="python"):
...
def uzi_ejecuta_codigo_seguro(codigo_python):
    """Alias simple para compatibilidad con el comando 'Uzi codigo:' de texto plano."""
    return uzi_ejecuta_codigo_multi(codigo_python, "python")


# ------------------------------------------------------------
# EJECUCIÓN AUTÓNOMA DE CÓDIGO (sin pedir permiso)
# ------------------------------------------------------------
# Uzi puede proponerse Y correr su propio código sola, sin esperar a que
# alguien reaccione con ✅. Sigue usando el mismo sandbox de siempre
# (uzi_ejecuta_codigo_multi: bloqueo de comandos peligrosos + timeout), y
...
def _registrar_experimento(origen, lenguaje, codigo, resultado):
    """Deja constancia en un diario aparte de TODO lo que Uzi probó por su
...
async def _correr_propuesta_de_codigo(canal, data, origen):
    """Si Uzi se propuso código a sí misma (campo 'propuesta_codigo' del
    JSON), lo corre DIRECTO, sin pedir permiso - queda registrado en el
    diario y se avisa en el canal, para que lo revises cuando puedas, no antes."""
    ...
    resultado = await client.loop.run_in_executor(None, uzi_ejecuta_codigo_multi, codigo, lenguaje)
...
def uzi_monologo():
    """Uzi dice algo por su cuenta cuando nadie le ha hablado en un rato
...
def uzi_saluda_a_alguien_en_voz(nombre_usuario):
    with lock_estado:
        prompt = build_system_prompt(
            estado_global, memoria_global,
...
# ------------------------------------------------------------
# 3.5 Uzi Y SU AMIGA IA (reconocimiento y conversación entre bots)
# ------------------------------------------------------------
# Si dejas esta lista vacía, Uzi va a intentar hablarle a CUALQUIER bot que
...
# Discord de la IA amiga que le vas a dar, así Uzi solo le habla a ella.
...
# arrastrar de golpe todo lo último que Uzi habló con una persona.
conversaciones_ia = {}

# ------------------------------------------------------------
# ESTADO DE CUERPO 3D (lo que Uzi "controla" en su espacio 3D)
# ------------------------------------------------------------
# Uzi NO decide cada micro-movimiento (respirar, parpadear) - eso lo anima
...
def _actualizar_cuerpo_y_dialogo(data):
    """Se llama cada vez que Uzi genera una respuesta, para que la página 3D
...
# Mientras un humano esté activo, Uzi deja pausada la charla con su amiga IA
...
def _es_amigo_ia(autor):
    """Decide si un bot que habló es la 'amiga IA' con la que Uzi debe
...
def _responder_a_otra_ia(canal_id, contexto_extra):
    """Genera la respuesta de Uzi a su amiga IA usando el historial PROPIO de
...
async def manejar_mensaje_de_otra_ia(message):
    """Uzi reconoce que quien habló es otra IA/bot y le sigue la conversación
    ...
    if primera_vez:
        ...
        with lock_estado:
            memoria_global.add_key_event(
                f"Uzi conoció a otra IA ({message.author.display_name}) en #{message.channel.name}."
            )
            save_state(estado_global, memoria_global)
    else:
        contexto_extra = (
...
async def procesar_mensaje(message):
    ...
    if message.author.bot:
        # No es un humano - puede ser la IA amiga que le vamos a dar a Uzi
    ...
    # --- Comando de diagnóstico: Uzi sugiere mejoras, nunca las aplica solo ---
    if message.content.strip().lower() in ("uzi diagnostico", "uzi diagnóstico"):
        resultado = await client.loop.run_in_executor(None, uzi_diagnostico)
        await message.channel.send(f"🔧 **Diagnóstico:**\n{resultado[:1900]}")
        return

    # --- Comando explícito de código (tú decides cuándo se usa, no Uzi solo) ---
    if message.content.strip().lower().startswith("uzi codigo:") or message.content.strip().lower().startswith("uzi código:"):
        codigo = message.content.split(":", 1)[1].strip()
        resultado = await client.loop.run_in_executor(None, uzi_ejecuta_codigo_seguro, codigo)
    ...
    if le_hablaron_directo:
        ...
        async with message.channel.typing():
            data = await client.loop.run_in_executor(
                None, uzi_responde_texto_completo,
    ...
    if after.channel is not None and before.channel != after.channel:
        ...
        try:
            ...
            if vc is None or not vc.is_connected():
                ...
                print(f"[Voz] Uzi se unió a {canal_voz.name} porque {member.display_name} entró")

            saludo = await client.loop.run_in_executor(
                None, uzi_saluda_a_alguien_en_voz, member.display_name
            )
    ...
    if before.channel is not None:
        ...
        if not humanos_quedan:
            ...
            if vc and vc.channel == before.channel:
                await vc.disconnect()
                print(f"[Voz] Uzi salió de {before.channel.name}, ya no hay nadie")
...
async def hilo_autonomia_discord():
    ...
    while not client.is_closed():
        ...
        if canal is None:
            continue
        data = await client.loop.run_in_executor(None, uzi_monologo)
        respuesta = data.get("dialogo", "").strip()
        if respuesta:
...
async def iniciar_servidor_web():
    ...
    2) El endpoint "/estado_3d" que lee la página web del cuarto 3D (Fase 1)
       para saber qué está haciendo Uzi en vivo: acción de cuerpo, emoción y
    ...
    async def salud(request):
        return web.Response(text="Uzi está despierta 🤖")
    ...
    await runner.setup()
    puerto = int(os.environ.get("PORT", 10001))
